...
#sorted function 
def extractsymbol(sentence):
    res=[] 
    if(sentence.symbol[0]!=""):
        res.append(sentence.symbol[0])
    else:
        for c in sentence.subexpressions:
            res=res+extractsymbol(c)
    res=set(res)
    res=list(res)
    return res
...
def TTEntails(kb,alpha,fname):
    sym1=extractsymbol(kb)
    sym2=extractsymbol(alpha)
    sym=sym1+sym2
    sym=set(sym)
    sym=list(sym)  
    smr,model=buildmodel(kb) 
    for i in smr:
        sym.remove(i)  
    return TTCheckall(kb,alpha,sym,model,0) 
def TTCheckall(kb,alpha,sym,model,count):   
    # if(count==0):
    #     print(model)
    if(len(sym)==0):  
        if(PLTrue(kb,model)):   
        ...
        model1[p]=True
        model2[p]=False 
        a=TTCheckall(kb,alpha,rest,model1,count+1) 
        b=TTCheckall(kb,alpha,rest,model2,count+1)
        return a and b
